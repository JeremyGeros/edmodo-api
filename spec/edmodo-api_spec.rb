...
describe Edmodo::API::Client do
  ...
  describe 'Invalid Requests' do
    ...
    it 'should not throw an error when a request doesnt return a 200 status code when silenced' do
      client = Edmodo::API::Client.new(@invalid_api_key, :silence_response_expections => true)

      expect(client.launch_requests("5c18c7")).to eq({"error" => {"code" => 3000, "message" => "Unauthorized API request"}})
    end
  end
